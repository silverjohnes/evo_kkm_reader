	...
	if len(line) > 14:  # проверка для обработки пустых строк
		...
		#
			...
			if command == 'ED':
				if len(line) == 23:
					if line[19:21] == "00":
						line = " ".join([line.strip(), "Получить уникальный номер контроллера (уникальный идентификатор платы в ККТ)\n"])
					if line[19:21] == "01":
						line = " ".join([line.strip(), "Хендшейк: Генерация и отправка сообщения в облако\n"])
					if line[19:21] == "02":
						line = " ".join([line.strip(), "Хендшейк: Прием сообщения из облака по частям\n"])
					if line[19:21] == "03":
